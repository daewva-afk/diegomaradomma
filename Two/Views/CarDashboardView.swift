...
struct CarDashboardView: View {
    ...
    private var carCard: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 0) {
                carCardStat(
                    label: "Total Spent",
                    value: "\(currencySymbol)\(String(format: "%.0f", vehicle.totalCost))",
                    icon: "dollarsign.circle"
                )
                Divider().frame(height: 32)
                carCardStat(
                    label: "Fill-Ups",
                    value: "\(vehicle.fillUps.count)",
                    icon: "fuelpump"
                )
                Divider().frame(height: 32)
                carCardStat(
    ...
    private var mileageText: String {
        if let maxOdo = vehicle.fillUps.map(\.odometer).max() {
            return "\(maxOdo) km"
        }
        return "-- km"
    }
    ...
    private var economyValue: String {
        guard let avg = vehicle.averageFuelEconomy else { return "--" }
        if useMetric {
    ...
    private var economyProgress: Double {
        guard let avg = vehicle.averageFuelEconomy else { return 0 }
        return max(0, min(1, (15 - avg) / 10.0))
    }

    private var costPerKmValue: String {
        let dist = vehicle.totalDistance
        guard dist > 0 else { return "--" }
        let costPerKm = vehicle.totalCost / Double(dist)
    ...
    private var nextServiceInfo: (type: MaintenanceType, daysLeft: Int)? {
        ...
        for type in MaintenanceType.allCases where type.recommendedIntervalDays > 0 {
            let lastRecord = vehicle.maintenanceRecords
                .filter { $0.type == type }
    ...
    private var fuelEconomyData: [(label: String, value: Double)] {
        let sorted = vehicle.fillUps.sorted { $0.odometer < $1.odometer }
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text("Recent Fill-Ups")
                    .font(.headline)
                Spacer()
                if !vehicle.sortedFillUps.isEmpty {
                    Text("\(vehicle.fillUps.count) total")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }

            let sorted = vehicle.sortedFillUps
            if sorted.isEmpty {
                HStack {
                    Spacer()
                    VStack(spacing: 8) {
                ...
                ForEach(Array(sorted.prefix(5).enumerated()), id: \.element.id) { index, fillUp in
                    FillUpRow(fillUp: fillUp, vehicle: vehicle, useMetric: useMetric, currencySymbol: currencySymbol)
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                if !vehicle.maintenanceRecords.isEmpty {
                    Text("\(vehicle.maintenanceRecords.count) records")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
